...
export default function ImportExcelPage() {
  ...
  async function handlePublishToFirebase() {
    if (!selectedFile || !teams.length) {
      setPublishMessage("กรุณาเลือกไฟล์ Excel และรอให้ Preview ข้อมูลสำเร็จก่อน");
      return;
    }
    ...
    try {
      ...
      setPublishMessage(
        `Publish ไม่สำเร็จ: ${firebaseError.message || "เกิดข้อผิดพลาดที่ไม่ทราบสาเหตุ"}`
      );
    } finally {
      setPublishing(false);
    }
    ...
    reader.onload = (loadEvent) => {
      try {
        ...
        const missingColumns = REQUIRED_COLUMNS.filter(
          const ADMIN_EMAIL = "sompronp@scg.com";
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          {fileName && (
            <p className="mt-5 rounded-lg border border-slate-700 bg-slate-950 p-4 text-sm">
              ไฟล์ที่เลือก:{' '}
              <strong className="text-cyan-300">{fileName}</strong>
            </p>
          )}
